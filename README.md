# Bamboo Agent
This is a customized base image for Atlassian's Bamboo Agent. It provides the ability to properly set the Bamboo URL. Atlassian's base image will set the URL on initial run then not bother ever updating the configuration even if you pass a new URL.
...
* `BAMBOO_URL` (default: NONE)

   The URL to your Bamboo instance.

## Notes
Bamboo will need it's "bamboo.jms.broker.client.uri" property set with the correct host-name. This should be your Bamboo host-name. It's located on the Bamboo server at `/var/atlassian/application-data/bamboo/bamboo.cfg.xml`.

# Original Bamboo Agent Readme
